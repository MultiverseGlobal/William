import type { MemoryNode, MemoryEdge, WorldModel, MemoryNodeType, MemoryRelation } from '@orion/types';
...
export async function extractEntitiesFromText(text: string, existingNodes: MemoryNode[]): Promise<void> {
  ...
  const systemPrompt = `You are Orion's memory system. Extract entities from the user's message and return structured JSON.
...
export async function getContextSummary(recentMessages: string[]): Promise<string> {
  ...
  return `[Orion's World Knowledge — top entities by confidence]\n${nodeLines}`;
}
