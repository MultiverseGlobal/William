import type { ActionRequest, ActionLog, Journey } from '@orion/types';
...
async function _coordinateAtlas(_actionId: string, req: ActionRequest): Promise<void> {
  ...
  const res = await fetch(`${process.env.ATLAS_API_URL}/api/coordinate`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ instruction, context, source: 'orion' })
  });
...
async function _organizeProject(_actionId: string, req: ActionRequest): Promise<void> {
  ...
  if (matched) {
    ...
    const newJ: Journey = {
      ...
      vision: 'Manage and coordinate through Orion companion actions.',
      progress: 0,
      milestones: [],
      memories: [],
      lessons: [],
