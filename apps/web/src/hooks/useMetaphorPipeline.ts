...
export function useMetaphorPipeline() {
  ...
  const pushActionLog = async (actionData: { title: string, summary: string, content: string }) => {
    const token = localStorage.getItem('metaphor_access_token');
    ...
    try {
      ...
      console.error("Failed to push action to Metaphor:", err);
      return false;
    }
  };

  return { brief, loading, pushActionLog };
}
