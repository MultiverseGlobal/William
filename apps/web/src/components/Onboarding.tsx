...
export const Onboarding: React.FC<OnboardingProps> = ({ onComplete }) => {
  ...
  const [showConnectMetaphor, setShowConnectMetaphor] = useState<boolean>(false);
  ...
  useEffect(() => {
    const runIntro = async () => {
      ...
      setMessages((prev) => [...prev, { id: 'm0', sender: 'orion', text: 'Hello.' }]);
      
      await new Promise((r) => setTimeout(r, 1600));
      setMessages((prev) => [...prev, { id: 'm1', sender: 'orion', text: "I'm Orion." }]);
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    setMessages((prev) => [...prev, { id: `w_${Date.now()}`, sender: 'orion', text: nextQuestion }]);
    setIsTyping(false);
  };
    ...
    const initialJourneys: Journey[] = [
      {
        ...
        currentState: 'Initializing project framework with Orion companion support.',
      ...
      >
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <div>
              <h2 style={{ fontSize: '0.9375rem', fontWeight: 600, letterSpacing: '-0.01em', margin: 0 }}>Orion Companion</h2>
        ...
        <div style={{
              ...
              return (
                <motion.div
                  ...
                  style={{
                    display: 'flex',
                    flexDirection: 'column',
                    alignItems: isOrion ? 'flex-start' : 'flex-end',
                    width: '100%'
                  }}
                >
                  {isOrion && (
                  ...
                  <div
                    style={{
                      padding: '14px 18px',
                      borderRadius: isOrion ? '4px 18px 18px 18px' : '18px 18px 4px 18px',
                      background: isOrion ? 'rgba(255, 255, 255, 0.03)' : 'var(--accent-color)',
                      backdropFilter: isOrion ? 'blur(12px)' : 'none',
                      border: isOrion ? '1px solid rgba(255, 255, 255, 0.05)' : 'none',
                      color: isOrion ? 'var(--text-primary)' : 'var(--accent-text)',
                      lineHeight: 1.5,
                      maxWidth: '85%',
                      fontSize: '0.9375rem',
          ...
          {isTyping && (
            ...
            >
              <div className="bg-white/5 border border-white/10 rounded-2xl px-5 py-4 text-[#a1a1aa] font-mono text-sm max-w-[80%] flex items-center gap-2 shadow-[0_0_15px_rgba(255,255,255,0.03)]">
                <div className="w-1.5 h-1.5 bg-[#d4d4d8] rounded-full animate-bounce [animation-delay:-0.3s]" />
                <div className="w-1.5 h-1.5 bg-[#d4d4d8] rounded-full animate-bounce [animation-delay:-0.15s]" />
                <div className="w-1.5 h-1.5 bg-[#d4d4d8] rounded-full animate-bounce" />
              </div>
            </motion.div>
          )}

          {showConnectMetaphor && !isTyping && (
            <motion.div
              initial={{ opacity: 0, y: 10 }}
            ...
            >
              <div className="flex flex-col gap-3 max-w-[80%]">
                ...
                <button
                  onClick={() => {
                    setShowConnectMetaphor(false);
                    handleComplete();
                  }}
                  className="text-[#a1a1aa] hover:text-white transition-colors text-sm font-medium text-left px-2"
                >
                  Skip for now
                </button>
              </div>
          ...
          {currentStep < 10 ? (
            <form onSubmit={handleSendMessage} style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
              <input
                ...
                placeholder={showConnectMetaphor ? "Please make a selection above..." : (currentStep === 0 ? "Wait for Orion..." : "Type your response...")}
                disabled={currentStep === 0 || isTyping || showConnectMetaphor}
                autoFocus
                style={{
                  flex: 1,
            ...
            >
              ...
              >
                <span>Enter Orion Companion</span>
                <ArrowRight size={18} />
              </button>
            </motion.div>
          )}
